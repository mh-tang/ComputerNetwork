...
3. 自身的相对序号`Seq`为0；确认号`Ack`为1，表示服务器期望接收客户端下一个报文的序号为1。
...
服务器端收到来自客户端确认收到服务器数据的报文后，得知从服务器到客户端的数据传输是正常的，三次握手完毕完毕，双方进入ESTABLISHED状态，可以开始传输数据。
...
紧接着，服务器还向客户端发送了一条带有[TCP segment of a reassembled PDU]标签的报文，意思是一个完整的TCP报文被拆分成了不同的段发送，因此两条报文的`Seq`和`Ack`一致。  
第二条报文包含了HTTP的响应信息，因此数据段长度为174。
...
可以看到，`Seq`和`Ack`顺序变化；HTTP报文分为了两个数据段响应头和响应体：给出了HTTP版本号，响应状态码，请求的数据等。
...
在请求logo.jpg之前，客户端和服务器建立了一个新的TCP连接，所以可以直接关闭获取HTML文件的50736端口的连接，使用50737端口进行资源请求。客户端多开一条TCP连接可能是用来减轻拥塞的影响，用单独的TCP连接来获取logo.jpg资源。  
...
`Seq`字段即为第一次握手的`Ack`值，`Ack`字段为第一次握手的`Seq`+1。

#### 第三次挥手
接着，客户端做好断开连接准备后，向服务器发送FIN+ACK报文，其中`Seq`和`Ack`字段与第二次握手中的值保持一致。  
