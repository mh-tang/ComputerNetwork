...
代码实现上，将套接字设置为非阻塞，然后进入循环，不断检查**发送、接收、超时**三个事件。核心逻辑上和上方状态机保持一致，下面给出核心代码，部分细节使用注释代替：  

```C++
    while (true) {
        // 判断当前分组序号是否在窗口内，注意序号范围
        ...
        if(temp < base+WINDOW_SIZE){  // 可以发送数据
            if (mPointer >= fileLength) {  // 文件发送完毕
                ...
                // 发送数据包，打印日志
                ...
                if (sendto(client, sendbuffer[nextSeq], (sizeof(header) + MAX_DATA_LENGTH), 0, 
                            (sockaddr*)&router_addr, rlen) == SOCKET_ERROR) {
        ...
        if(getData > 0){
            ...
            if (header.flag == ACK && check((u_short*)&header, sizeof(header) == 0)) {
                ...
                // 打印窗口滑动日志；若窗口滑动后窗口内还有未确认数据包，重置计时器
                ...     
            }
            // 数据包错误什么也不做
        }
