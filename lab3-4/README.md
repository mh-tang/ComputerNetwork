...
- 但如果时延设置过大，因为SR对数据包需要做的处理较多，可能会出现大量超时丢包的情况，**逼急与GBN的重传整个窗口**。且没有累积确认的性质，性能可能更差。

### 有待改进
由于`router`程序对服务器端返回确认`ACK`不会进行丢包处理，所以GBN**累积确认的性能优势**在本次实验中无法体现。  
可以想象，如果服务器端返回的确认`ACK`发生丢包，SR的性能毫无疑问会受到影响，而GBN却能通过累积确认的方式继续发送数据，性能优势就会体现出来。